    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">เพิ่มสินค้า</h1>
                                        ...
                                        <? if($pid){ ?>{{ Form::hidden('s_name', $pid) }}<? } ?>
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="s_name" name="s_name" <? if($pid){ echo 'disabled';} ?>>
                                                    ...
                                                    <option value="">- เลือกชื่อสินค้า (บาร์โค้ด) -</option>
                                                    @foreach($sproduct as $pro)
                                                    <? if($pro->p_id == $pid) { $ena = ' selected';}else{ $ena = '';} ?>
                                                    <option value="{{ $pro->p_id }}" {{ $ena }}>{{ $pro->p_name . ' (' . $pro->p_barcode . ')' }}</option>
                                                    @endforeach
                                            ...
                                            <div class="form-group">
                                                {!! Form::label('t_num','จำนวนสินค้า'); !!}
                                                {!! Form::text('t_num',null, ['class'=>'form-control','placeholder'=>'ระบุจำนวน เช่น 20']); !!}
                                            </div>
