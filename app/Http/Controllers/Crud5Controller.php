...
class Crud5Controller extends Controller
{
    public function index(Request $request)
    {
        ...
        $request->session()->put('u_status', $request->has('u_status') ? $request->get('u_status') : ($request->session()->has('u_status') ? $request->session()->get('u_status') : -1));
        ...
        if ($request->session()->get('u_status') != -1)
            $customers = $customers->where('u_status', $request->session()->get('u_status'));
        $customers = $customers->where('u_name', 'like', '%' . $request->session()->get('search') . '%')
    ...
    public function create(Request $request)
    {
        if ($request->isMethod('get'))
            ...
            $customer->u_status = $request->u_status;
